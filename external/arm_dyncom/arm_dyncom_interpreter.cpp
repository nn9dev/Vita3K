...
static unsigned int InterpreterTranslateInstruction(const ARMul_State* cpu, const u32 phys_addr,
    ...
    if (DecodeARMInstruction(inst, &idx) == ARMDecodeStatus::FAILURE) {
        LOG_ERROR("Decode failure.\tPC: [{:#010X}]\tInstruction: {:08X}", phys_addr,
                  inst);
        LOG_ERROR("cpsr={:#X}, cpu->TFlag={}, r15={:#010X}", cpu->Cpsr, cpu->TFlag,
                  cpu->Reg[15]);
        CITRA_IGNORE_EXIT(-1);
    }
...
MCRR_INST: {
    ...
    if (inst_base->cond == ConditionCode::AL || CondPassed(cpu, inst_base->cond)) {
        ...
        LOG_ERROR("MCRR executed | Coprocessor: {}, CRm {}, opc1: {}, Rt: {}, Rt2: {}",
...
MRRC_INST: {
    ...
    if (inst_base->cond == ConditionCode::AL || CondPassed(cpu, inst_base->cond)) {
        ...
        LOG_ERROR("MRRC executed | Coprocessor: {}, CRm {}, opc1: {}, Rt: {}, Rt2: {}",
                  inst_cream->cp_num, inst_cream->crm, inst_cream->opcode_1, inst_cream->rt,
                  inst_cream->rt2);
    }
