...
#include <arm_dyncom/skyeye_common/vfp/vfp_helper.h>
...
static void vfp_single_dump(const char* str, struct vfp_single* s) {
    LOG_TRACE("{}: sign={} exponent={} significand={:08x}", str, s->sign != 0,
...
u32 vfp_single_normaliseround(ARMul_State* state, int sd, struct vfp_single* vs, u32 fpscr,
    ...
    LOG_TRACE("rounding increment = 0x{:08x}", incr);
...
static u32 vfp_single_ftoui(ARMul_State* state, int sd, int unused, s32 m, u32 fpscr) {
    ...
    LOG_TRACE("ftoui: d(s{})={:08x} exceptions={:08x}", sd, d, exceptions);
...
static u32 vfp_single_ftosi(ARMul_State* state, int sd, int unused, s32 m, u32 fpscr) {
    ...
    LOG_TRACE("ftosi: d(s{})={:08x} exceptions={:08x}", sd, d, exceptions);
...
u32 vfp_single_cpdo(ARMul_State* state, u32 inst, u32 fpscr) {
    ...
    LOG_TRACE("vecstride={} veclen={}", vecstride, (veclen >> FPSCR_LENGTH_BIT) + 1);

    if (!fop->fn) {
        LOG_CRITICAL("could not find single op {}, inst=0x{:x}@0x{:x}",
    ...
    for (vecitr = 0; vecitr <= veclen; vecitr += 1 << FPSCR_LENGTH_BIT) {
        ...
        if (op == FOP_EXT)
            LOG_TRACE("itr{} ({}{}) = op[{}] (s{}={:08x})", vecitr >> FPSCR_LENGTH_BIT,
                      type, dest, sn, sm, m);
        else
            LOG_TRACE("itr{} ({}{}) = (s{}) op[{}] (s{}={:08x})",
                      vecitr >> FPSCR_LENGTH_BIT, type, dest, sn, FOP_TO_IDX(op), sm, m);

        except = fop->fn(state, dest, sn, m, fpscr);
        LOG_TRACE("itr{}: exceptions={:08x}", vecitr >> FPSCR_LENGTH_BIT, except);
