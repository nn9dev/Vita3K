...
static void vfp_double_dump(const char* str, struct vfp_double* d) {
    LOG_TRACE("VFP: {}: sign={} exponent={} significand={:016x}", str, d->sign != 0,
...
u32 vfp_double_normaliseround(ARMul_State* state, int dd, struct vfp_double* vd, u32 fpscr,
    ...
    LOG_TRACE("VFP: rounding increment = 0x{:08x}", incr);
...
static u32 vfp_compare(ARMul_State* state, int dd, int signal_on_qnan, s64 m, u32 fpscr) {
    ...
    LOG_TRACE("In {}, state=0x{}, fpscr=0x{:x}", __FUNCTION__, fmt::ptr(state), fpscr);
    ...
    LOG_TRACE("In {}, state=0x{}, ret=0x{:x}", __FUNCTION__, fmt::ptr(state), ret);
...
static u32 vfp_double_ftoui(ARMul_State* state, int sd, int unused, int dm, u32 fpscr) {
    ...
    LOG_TRACE("VFP: ftoui: d(s{})={:08x} exceptions={:08x}", sd, d, exceptions);
...
static u32 vfp_double_ftosi(ARMul_State* state, int sd, int unused, int dm, u32 fpscr) {
    ...
    LOG_TRACE("VFP: ftosi: d(s{})={:08x} exceptions={:08x}", sd, d, exceptions);
...
u32 vfp_double_add(struct vfp_double* vdd, struct vfp_double* vdn, struct vfp_double* vdm,
    ...
    if (vdn->significand & (1ULL << 63) || vdm->significand & (1ULL << 63)) {
        LOG_INFO("VFP: bad FP values in {}", __func__);
...
u32 vfp_double_cpdo(ARMul_State* state, u32 inst, u32 fpscr) {
    ...
    LOG_TRACE("VFP: vecstride={} veclen={}", vecstride,
              (veclen >> FPSCR_LENGTH_BIT) + 1);

    if (!fop->fn) {
        LOG_TRACE("VFP: could not find double op {}", FEXT_TO_IDX(inst));
        goto invalid;
    }
    ...
    for (vecitr = 0; vecitr <= veclen; vecitr += 1 << FPSCR_LENGTH_BIT) {
        ...
        if (op == FOP_EXT)
            LOG_TRACE("VFP: itr{} ({}{}) = op[{}] (d{})", vecitr >> FPSCR_LENGTH_BIT,
                      type, dest, dn, dm);
        else
            LOG_TRACE("VFP: itr{} ({}{}) = (d{}) op[{}] (d{})",
                      vecitr >> FPSCR_LENGTH_BIT, type, dest, dn, FOP_TO_IDX(op), dm);

        except = fop->fn(state, dest, dn, dm, fpscr);
        LOG_TRACE("VFP: itr{}: exceptions={:08x}", vecitr >> FPSCR_LENGTH_BIT, except);
