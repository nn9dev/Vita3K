...
ARMul_State::ARMul_State(MemState* mem_, PrivilegeMode initial_mode)
    : mem{mem_} {
    Reset();
...
static T DoReadMemory(const ARMul_State* cpu, u32 address) {
    Ptr<T> ptr{ address };
    if (!cpu->fastmem || cpu->log_mem) {
        if (!ptr || !ptr.valid(*cpu->mem) || ptr.address() < cpu->mem->host_page_size) {
            LOG_ERROR("DynCom: invalid read of u{} at 0x{:08X} (PC 0x{:08X})",
                      sizeof(T) * 8, address, cpu->Reg[15]);
            return T{};
        }
    }

    const T value = *ptr.get(*cpu->mem);
    if (cpu->log_mem)
        LOG_TRACE("DynCom: read u{} at 0x{:08X} = 0x{:X}", sizeof(T) * 8, address, value);
    return value;
}

template <typename T>
static void DoWriteMemory(ARMul_State* cpu, u32 address, T value) {
    Ptr<T> ptr{ address };
    if (!cpu->fastmem || cpu->log_mem) {
        if (!ptr || !ptr.valid(*cpu->mem) || ptr.address() < cpu->mem->host_page_size) {
            LOG_ERROR("DynCom: invalid write of u{} at 0x{:08X} = 0x{:X} (PC 0x{:08X})",
                      sizeof(T) * 8, address, value, cpu->Reg[15]);
            return;
        }
    }

    *ptr.get(*cpu->mem) = value;
    if (cpu->log_mem)
        LOG_TRACE("DynCom: write u{} at 0x{:08X} = 0x{:X}", sizeof(T) * 8, address, value);
}

u8 ARMul_State::ReadMemory8(u32 address) const {
    return DoReadMemory<u8>(this, address);
}

u16 ARMul_State::ReadMemory16(u32 address) const {
    u16 data = DoReadMemory<u16>(this, address);
...
u32 ARMul_State::ReadMemory32(u32 address) const {
    u32 data = DoReadMemory<u32>(this, address);
...
u64 ARMul_State::ReadMemory64(u32 address) const {
    u64 data = DoReadMemory<u64>(this, address);
...
void ARMul_State::WriteMemory8(u32 address, u8 data) {
    DoWriteMemory<u8>(this, address, data);
}
...
void ARMul_State::WriteMemory16(u32 address, u16 data) {
    ...
    DoWriteMemory<u16>(this, address, data);
}
...
void ARMul_State::WriteMemory32(u32 address, u32 data) {
    ...
    DoWriteMemory<u32>(this, address, data);
}
...
void ARMul_State::WriteMemory64(u32 address, u64 data) {
    ...
    DoWriteMemory<u64>(this, address, data);
}
...
u32 ARMul_State::ReadCP15Register(u32 crn, u32 opcode_1, u32 crm, u32 opcode_2) const {
    ...
    LOG_ERROR("MRC CRn={}, CRm={}, OP1={} OP2={} is not implemented. Returning zero.",
    ...
    if (InAPrivilegedMode()) {
        if (crn == 1 && opcode_1 == 0 && crm == 0) {
            ...
            if (crm == 0 && opcode_2 == 4) {
                ...
                LOG_ERROR("Unimplemented virtual to physical address");
            } else if (crm == 5) {
                if (opcode_2 == 0)
